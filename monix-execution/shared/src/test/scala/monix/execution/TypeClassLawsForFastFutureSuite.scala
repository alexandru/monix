...
object TypeClassLawsForFastFutureSuite extends BaseLawsSuite {
  checkAllAsync("CoflatMap[FastFuture]") { implicit ec =>
    CoflatMapTests[FastFuture].coflatMap[Int,Int,Int]
  }

  checkAllAsync("MonadError[FastFuture, Throwable]") { implicit ec =>
    MonadErrorTests[FastFuture, Throwable].monadError[Int,Int,Int]
  }

  test("adaptError") {
    implicit val ec = TestScheduler()
    val F = MonadError[FastFuture, Throwable]
