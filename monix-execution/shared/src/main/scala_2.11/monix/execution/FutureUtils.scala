...
object FutureUtils {
  ...
  def transform[A,B](source: Future[A], f: Try[A] => Try[B])(implicit ec: ExecutionContext): Future[B] =
    source match {
      case ref: MonixFuture[_] =>
        // Monix's implementations already have transform
        ref.asInstanceOf[MonixFuture[A]].transform(f)(ec)
  ...
  def transformWith[A,B](source: Future[A], f: Try[A] => Future[B])(implicit ec: ExecutionContext): Future[B] =
    source match {
      case ref: MonixFuture[_] =>
        // Monix's implementations already have transformWith
        ref.asInstanceOf[MonixFuture[A]].transformWith(f)(ec)

      case _ =>
        val promise = FastFuture.promise[B]
        source.onComplete { result =>
          val fb = try f(result) catch { case NonFatal(t) => FastFuture.failed(t) }
          promise.completeWith(fb)
        }
        promise
    }
