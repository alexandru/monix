...
/** A [[scala.concurrent.Future]] re-implementation that makes use of
  * a couple of tricks to achieve better throughput, at the expense
  * of fairness.
  *
  * `FastFuture` exposes [[FastFuture#onCompleteLight onCompleteLight]], as
  * an alternative to normal `onComplete`. This `onCompleteLight` method
  * uses the
  * [[monix.execution.schedulers.TrampolinedRunnable TrampolinedRunnable]]
  * mechanism implemented in
  * [[monix.execution.schedulers.TrampolineScheduler TrampolineScheduler]]
  * in order to trigger light asynchronous boundaries that get executed
  * on the current thread and callstack, by using a thread-local trampoline.
  *
  * All `FastFuture` operations like `map`, `flatMap`, `transformWith`, etc.
  * are implemented in terms of [[FastFuture#onCompleteLight onCompleteLight]],
  * therefore these operations aren't doing logical forking on execution,
  * but still preserve stack safety.
  *
  * The [[onComplete]] operation on the other hand does trigger an asynchronous
  * boundary, just like regular [[scala.concurrent.Future.onComplete onComplete]].
  *
  ...
  */
abstract class FastFuture[+A] extends Future[A] {
...
object FastFuture {
  /** Executes the given expression asynchronously (managed by the
    ...
    *
    * See [[eval]] for working only with a light async boundary.
    */
  def apply[A](thunk: => A)(implicit ec: ExecutionContext): FastFuture[A] = {
    val promise = new LightPromise[A]()
    ec.execute(new Runnable {
      def run(): Unit =
        promise.complete(try Success(thunk) catch { case NonFatal(e) => Failure(e) })
    })
    promise
  }

  /** Evaluates the given expression in the context of the given
    * `ExecutionContext` with only a light async boundary.
    *
    * Like [[apply]], but for
    * [[monix.execution.schedulers.TrampolineScheduler supporting schedulers]]
    * this will execute the given `thunk` on the current thread and callstack.
    */
  def eval[A](thunk: => A)(implicit ec: ExecutionContext): FastFuture[A] = {
    val promise = new LightPromise[A]()
  ...
  def fromFuture[A](f: Future[A]): FastFuture[A] =
    f match {
      ...
      case _ =>
        val promise = new LightPromise[A]()
        f.onComplete(promise.complete)(immediate)
        promise
    }
  ...
  final class LightPromise[A] private (private[this] val state: AtomicAny[AnyRef], isEmpty: Boolean)
    ...
    /** Default constructor.
      ...
      *        to apply to the internal atomic reference that is used for
    ...
    @tailrec private def link(target: LightPromise[A]): Unit = if (this ne target) {
      state.get match {
        case null =>
          if (!state.compareAndSet(null, target)) {
            // $COVERAGE-OFF$
            link(target)
            // $COVERAGE-ON$
          }
    ...
    @tailrec private def dispatchOrAddListener(ref: Listener): Unit =
      state.get match {
        ...
        case prev: Listener =>
          if (!state.compareAndSet(prev, ref :: prev :: Nil)) {
            // $COVERAGE-OFF$
            dispatchOrAddListener(ref) // retry
            // $COVERAGE-ON$
          }
    ...
    @tailrec private def dispatchOrAddListeners(list: List[Listener]): Unit =
      state.get match {
        ...
        case null =>
          if (!state.compareAndSet(null, list)) {
            // $COVERAGE-OFF$
            dispatchOrAddListeners(list) // retry
            // $COVERAGE-ON$
          }
        case prev: Listener =>
          if (!state.compareAndSet(prev, prev :: list)) {
            // $COVERAGE-OFF$
            dispatchOrAddListeners(list) // retry
            // $COVERAGE-ON$
          }
  ...
  object LightPromise {
    /** Creates an already completed [[LightPromise]].
      *
      * The equivalent of Scala's `Promise.successful`.
      */
