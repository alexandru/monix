...
import monix.execution.{Cancelable, FastFuture, Scheduler, UncaughtExceptionReporter}
...
abstract class ExecutorScheduler(e: ExecutorService, r: UncaughtExceptionReporter)
  ...
  override final def awaitTermination(timeout: Long, unit: TimeUnit, awaitOn: Scheduler): Future[Boolean] = {
    val promise = FastFuture.promise[Boolean]
    awaitOn.execute(new Runnable {
      override def run() =
        try blocking {
          promise.success(e.awaitTermination(timeout, unit))
        }
        catch {
          case NonFatal(ex) =>
            promise.failure(ex)
